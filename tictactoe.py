...
def opponentAboutToWin(board, opponent_symbol, player_symbol):
...
def computer_move(board, user_symbol):
    ...
    if opponentAboutToWin(board, computer_symbol, user_symbol):
        best_spot = opponentAboutToWin(board, computer_symbol, user_symbol)
        board[best_spot - 1] = computer_symbol
        return showBoard(board)

    if opponentAboutToWin(board, user_symbol, computer_symbol):
...
def game():
    ...
    while True:
        ...
        if not user_move == 'Invalid input. Please try again':
			print(computer_move(board, user_symbol))
